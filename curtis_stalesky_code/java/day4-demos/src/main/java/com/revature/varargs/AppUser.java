...
public class AppUser {
    ...
    // Variables arguments
    /*
        - ... means you are taking in a variable number of argument for this time
    ...
    public void addRoles(Role... newRoles) {
        ...
        if (roles == null ) {
            ...
            // If not, instantiate a new Role[] with a length equal to newRoles.length
            roles = newRoles;
            return; // doesn't actually return anything, just leaves this method early
        }

        //  Make an Int that holds index position that we will start adding new roles
        int startingIndexForNewRoles = roles.length;

        // Make a copy of existing roles array but with length equal to (newRoles.length + roles.length)
        roles = Arrays.copyOf(roles, newRoles.length + roles.length);

        // Loop across the newRoles and add each new role into the roles array starting at the index we obtained
